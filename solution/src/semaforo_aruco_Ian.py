...
light_state = rospy.Publisher('/light_state', String, queue_size = 1)
posPub = rospy.Publisher('/marker_properties', Vector3, queue_size = 1)
...
def process_frame():
    ...
    numCirclesG=cv.HoughCircles(grayG,cv.HOUGH_GRADIENT,1,50,param1=60,param2=50,minRadius=100,maxRadius=200) #se puede mover el maxRadius

    #procesamos Rojos
    maskR1 = cv.inRange(frame, colorLowR1, colorHighR1)
    maskR2 = cv.inRange(frame, colorLowR2, colorHighR2)
    ...
    numCirclesR=cv.HoughCircles(grayR,cv.HOUGH_GRADIENT,1,50,param1=60,param2=50,minRadius=100,maxRadius=200) #se puede mover el maxRadius
